...
def files_to_array(source_name):
    files = []
...
def convert_data(source_name, new_h, new_w):
    """
    Resizes all images in a given folder to a new height and width May take a few minutes depending on how many
    images are in the file

    ONLY WORKS IF THERE ARE NO SUBDIRECTORIES AND THE DIRECTORY CHOSEN LEADS TO ONLY THE IMAGES
    Warning: Will delete any data that it cannnot recognize when reading an image using opencv
            Args:
    ...
    files = files_to_array(source_name)

    for i in range(len(files)):
        imagename = files[i]
        print(files[i])
        print(f"Percentage Complete: %{(i / len(files)) * 100}")
